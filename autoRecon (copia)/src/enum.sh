...
function nmap_enum(){
    ...
    nmap -sV -sC -p$ports_tcp $TARGET -oN targets/$TARGET/vulns/nmap_vulns &>/dev/null
    data "Archivo de vulnerabilidades con Nmap" "nmap_vulns -> (targets/$TARGET/vulns/nmap_vulns)"
}

function searchsploit_enum(){
    searchsploit -j --nmap targets/$TARGET/scans/TCPServices.xml > targets/$TARGET/services/searchsploit_nmap 2>/dev/null
...
function http_enum(){
    ...
    ffuf -ic -w /usr/share/dirbuster/wordlists/directory-list-2.3-medium.txt -e '$extensions' -u http://${TARGET}:$ports_http/FUZZ | tee targets/$TARGET/vulns/ffuf_extensions_vulns
    ...
    gobuster dir -re -t 65 -u http://$TARGET:$ports_http -w /usr/share/dirbuster/wordlists/directory-list-2.3-medium.txt -o targets/$TARGET/vulns/gobuster_vulns -k
