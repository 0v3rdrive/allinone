...
function Nmap_TCP(){
    nmap -p- --open --min-rate 5000 -vvv -Pn -n $TARGET -oA targets/$TARGET/scans/TCPports &> /dev/null
    if [ $? -eq 0 ]; then
...
function Nmap_UDP(){
    nmap -sU --min-rate 5000 -p- --open -Pn -n -vvv $TARGET -oA targets/$TARGET/scans/UDPports &> /dev/null
    if [ $? -eq 0 ]; then
